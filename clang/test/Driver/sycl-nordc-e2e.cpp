// End-to-end test for -fno-sycl-rdc: build a two translation unit program,
// check that each object carries its own finalized device image, then link and
// run the program to verify that both images reach the SYCL runtime.
//
...
// RUN: %clangxx -fsycl -fno-sycl-rdc -c %S/Inputs/sycl-nordc-e2e-second-tu.cpp \
// RUN:   -o %t/second.o

// Each object holds a finalized image in ".sycl_fatbin"; nothing is left for a
// later device link step in ".llvm.offloading".
// RUN: llvm-readelf -S %t/main.o %t/second.o | FileCheck %s \
// RUN:   --check-prefix=SECTIONS --implicit-check-not='.llvm.offloading'
// SECTIONS: .sycl_fatbin
// SECTIONS: .sycl_fatbin
...
// Each image is finalized from a single translation unit, so it offers only the
// kernels of that translation unit.
...
// RUN:   --implicit-check-not=second_tu_kernel_name
// RUN: FileCheck %s --check-prefix=SECOND-IMAGE --input-file=%t/second.image \
// RUN:   --implicit-check-not=main_tu_kernel_name
// MAIN-IMAGE-DAG: spirv64-unknown-unknown
// MAIN-IMAGE-DAG: main_tu_kernel_name
// MAIN-IMAGE-DAG: main_tu_other_kernel_name
// SECOND-IMAGE-DAG: spirv64-unknown-unknown
// SECOND-IMAGE-DAG: second_tu_kernel_name

// The finalized image is one offload binary holding one entry, tagged with the
// device it was finalized for. No device was asked for, which is spelled as an
// empty architecture.
...
// MAIN-ENTRIES:      arch{{ *$}}
// MAIN-ENTRIES:      triple{{ *}}spirv64-unknown-unknown
// MAIN-ENTRIES:      producer{{ *}}sycl
// MAIN-ENTRIES-NOT:  OFFLOADING IMAGE

// Splitting the device code of a translation unit by kernel gives it an image
// per kernel. They have to be merged into a single offload binary, because that
// is what the runtime is handed and it reads one header: were they concatenated,
// a reader would stop after the first one and see image [0] twice.
...
// Linking keeps the finalized images as they are.
// RUN: %clangxx -fsycl -fno-sycl-rdc -nolibsycl %t/main.o %t/second.o -o %t/nordc
// RUN: llvm-readelf -S %t/nordc | FileCheck %s --check-prefix=EXE-SECTIONS \
// RUN:   --implicit-check-not='.llvm.offloading'
// EXE-SECTIONS: .sycl_fatbin
...
// RUN: %clangxx -fsycl -fsycl-rdc -c %S/Inputs/sycl-nordc-e2e-second-tu.cpp \
// RUN:   -o %t/second.rdc.o
...
// A second kernel of the same translation unit, so that splitting the device
// code by kernel has something to split.
