...
/// Check the phases graph in non-RDC mode: the device images of a translation
/// unit are packaged and finalized into a fat binary of their own, which is
/// included into the host compilation. Only the host still needs the final
/// link-time clang-linker-wrapper.
...
/// architecture, and a single fat binary is still what reaches the host.
...
/// and must not use the compile-time include path.
...
/// In non-RDC mode the split does happen while compiling, because that is when
/// the device images are finalized.
