...
// -fno-sycl-rdc each translation unit is finalized into a device image of its
// own, so both images have to be registered and both kernels have to be found
// at run time. An RDC build of the same sources has to give the same result.
...
#include "Inputs/no_rdc_multi_tu.hpp"
...
int main() {
  ...
  // Wait in between so that the result does not depend on whether the queue
  ...
  for (std::size_t I = 0; I < N; ++I)
    assert(Data[I] == (static_cast<int>(I) + 1) * 3);
