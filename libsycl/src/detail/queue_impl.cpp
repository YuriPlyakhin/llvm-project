...
static bool checkEventsPlatformMatch(std::vector<std::shared_ptr<EventImpl>> &Events,
  ...
  return std::all_of(Events.cbegin(), Events.cend(),
                     [&QueuePlatform](const std::shared_ptr<EventImpl> &Event) {
                       return &Event->getPlatformImpl() == &QueuePlatform;
                     });
}

void QueueImpl::setKernelParameters(std::vector<std::shared_ptr<EventImpl>> &&Events,
...
void QueueImpl::submitKernelImpl(std::string_view KernelName,
  ...
  auto Result =
      olLaunchKernel(MOffloadQueue, MDevice.getOLHandle(), Kernel, Arguments,
